...
notify-send --app-name="DONE" "Fonts folder copied into $(whoami)" "$font_dest" -u normal

# Move the scripts folder and its contents
cp -r "$script_folder" "$script_dest"
notify-send --app-name="DONE" "Script folder copied into $(whoami)" "$script_dest" -u normal

# Move the wallpapers folder and its contents
cp -r "$wallpapers_src" "$wallpapers_dest"
notify-send --app-name="DONE" "Wallpaper folder moved into $(whoami)" "$wallpapers_dest" -u normal
